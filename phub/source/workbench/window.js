...
!function(){
	 ...
	 * 
	 * @param {{}} conf - BrowserWindow options
	 */
	var get_win = function(name, conf){
		if (!conf) {
			conf = {};
		}
		try{
			var c = require(path.join(PATH.UI_CONF, name+'.json'));
			conf = Object.assign(c, conf);
		}catch(e){}
		conf = Object.assign({
			width: 800,
			height: 600,
	...
	var win_load = function(win, name, param){
		if(win){
			...
			var page_url = path.join('file://' , PATH.UI, name+ '.html');
	...
	function openMain() {
		var win = open('main');
		ipcMain.once('main.ready', function () {
			win.show();
		});
	}
	/**
	...
	function _focusLastOfUi() {
		...
		for (var i = wins.length - 1; i>=0; i--) {
			...
			if (w) {
				var url = w.getURL();
				if (!/(service|run|autowork_polling)\.\w+$/.test(url)) {
	...
	/**
	 * UI进程是否已经打开（默认login.html或main.html是否打开）
	 * 
	 * @param {string} [name='login|main']
	 * @returns {BrowserWindow|undefined}
	 */
	function _isOpenedUi(name) {
		if (name === undefined) {
			name = 'login|main';
		}
		var r = new RegExp('(' + name + ')\\.\\w+$');
		var wins = BrowserWindow.getAllWindows();
		for (var i = wins.length - 1; i>=0; i--) {
			var w = wins[i];
			if (w) {
